...
class ShareModalComponent extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.generateLink(this.state.permission);
  }
  ...
  generateLink = async (permission: DiagramAccess) => {
    const response = await LinkRepository.generateLink(this.props.diagram, permission);
    this.setState({ link: response.link });
  };

  changePermission = (permission: DiagramAccess) => {
    this.setState({ permission });
    this.generateLink(permission);
  };
    ...
    return createPortal(
      <Modal id="share-modal" centered show={show} size="lg" onHide={this.handleClose} dialogClassName="w-75">
        ...
        <Modal.Body>
          ...
          <InputGroup className="mb-3">
            <FormControl readOnly value={this.state.link} />
